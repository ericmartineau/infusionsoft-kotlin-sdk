...
open class ApiClient(val baseUrl: String, bearerToken:String) {
    ...
    inline protected fun <reified T:Any> serializer() :KSerializer<T> {
      val serializerFn = T::class.companionObject
          ?.memberFunctions?.firstOrNull {
        it.name == "serializer"
      }
      return serializerFn?.call(T::class.companionObjectInstance) as KSerializer<T>
    }
    ...
    inline protected fun <reified T:Any> responseBody(response: Response, mediaType: String = JsonMediaType): T? {
      ...
      return when(inbound) {
        null-> null
          ...
          val parsed = json.parse<T>(serializer(), inbound)
          return parsed
        }
      }
